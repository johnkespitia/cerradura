...
import { get, save } from '../../store/guardSlice'
...
const GuardPage = () => {
  ...
  const store = async (formData) => {
    try {
      ...
      if(guardEditable.id === undefined){
        res = await axios.post(process.env.REACT_APP_USER_API_URL + "/guard", formData,configs)
      }else{
        res = await axios.put(process.env.REACT_APP_USER_API_URL + `/guard/${guardEditable.id}`, formData,configs)
      }
  ...
  return <>
  <h3 className={"mb-4 mt-2"}>Aplicaciones</h3>
    <hr />
    <List loading={loading} handleEdit={handleEdit}/>
    <CreateButton clickEvent={newGuardForm}/>
    <ModalOption show={show} setClose={closeModal} title={"Administrar Aplicación"}>
